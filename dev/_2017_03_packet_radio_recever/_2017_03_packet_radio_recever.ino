...
int LED  = 6;
int errorSize = 5;
int errorPattern[5] = {1, 0, 1, 0, 1};

void onOff(int pin, bool state,  int interval){
...
void pattern( int arr[], int size, int interval){

  for(int i = 0; i < size -1; i++){
    onOff(LED, HIGH, interval);
    onOff(LED, LOW, interval);
...
void loop()
{
  if (rf95.available())
  {
    ...
    {
      pattern(errorPattern, errorSize, 1000);
